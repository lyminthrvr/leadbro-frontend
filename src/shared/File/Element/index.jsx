...
const FileElement = ({key,file,onDelete}) => {
    ...
    return (
        <div onClick={handleDownload} className={styles.file}>
            <div>
                <Icon viewBox={'0 0 18 20'}  fill={'#FF6A55'} classname={styles.file_icon} name={'file'} size={18}/>
            </div>
            <div className={styles.file_name}>
                <span>{file.name}</span>
            </div>
            {onDelete && <div onClick={(e)=>{
                e.stopPropagation()
                e.preventDefault()
                onDelete(file.id)
            }}>
                <Icon fill={'#6F767E'} name={'close'} size={14} viewBox={'0 0 15 15'}/>
            </div>}
