...
const ClientComments = ({comments,onChange}) => {
    ...
    function countFiles() {
        return Object.values(comments??{}).reduce((totalFiles, comment) => {
            return totalFiles + (comment.value.files ? comment.value.files.length : 0);
        }, 0);
    }
    ...
    return (
        <Card>
            ...
            <CommentsFilters filterComments={handleFilterByComments} filterFiles={handleFilterByFiles} filterAll={handleFilterAll} filesLength={countFiles()} commentsLength={countComments()}/>
            <CommentsList  filterFiles={isFilterFiles} filterComments={isFilterComments} comments={comments}/>
        </Card>
    );
};
